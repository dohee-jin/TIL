...
- 프론트엔드
    ...
    - 스켈레톤 컴포넌트 배치   
    > 스켈레톤 컴포넌트란?    
    데이터를 가져오는 동안 콘텐츠를 표시하는 컴포넌트를 의미한다. 사용자는 콘텐츠를 기다리다가 쉽게 지치고 지루함을 느끼므로 빈 화면면 보여주게되면 떠나가게 된다.   

    로딩바, 스피너는 데이터가 로딩되고 있음을 보여주고 사용자가 페이지나 컴포넌트가 로딩되고 있음을 이해하는데 도움이 된다.   

    요즘은 로딩바, 스피너 보다는 대체 UI 컴포넌트를 보여주는 추세다.   
    ![alt text](/img/image-25.png)   
    
    데이터 로딩이 끝나면 스켈레톤을 사라지게 해야하기 때문에 로딩 상태를 상태변수로 관리하여   
    로딩이 끝나면 스켈레톤이 사라지게 만들어야 한다.   
    패치 요청 상태에 따라 setLoading 으로 상태를 변경하고 조건부 렌더링으로 스켈레톤을 렌더링 하고 있다.
    ```
    // 로딩 상태 관리
...
#### 🛠️ 문제와 원인
- 문제: 삭제 요청 실페
- 원인: DeleteMapping("/{id}") 와 @PathVariable Long eventId 경로 변수 이름과 메소드 매개변수 이름이 불일치하여 발생
```
// 삭제 요청
@DeleteMapping("/{id}")
public ResponseEntity<?> deleteEvent(@PathVariable Long eventId) {
    ...
    return ResponseEntity.ok().body(Map.of(
            "message", "이벤트가 삭제되었습니다. id - " + eventId
    ));
}
```
...
```
경로 변수 이름과 매개변수의 이름을 일치시켜 문제 해결

---

#### 🛠️ 문제와 원인
- 문제: 이벤트 단일 조회를 요청했는데 JSON 데이터가 아닌 HTML(index.html)이 반환
- 원인: fetch의 URL 작성 오류   
백틱 `` 안에 불필요한 작은 따옴표가 들어가서 API 서버로 요청을 보내지 않고 잘못된 url로 요청을 보내서 문제가 발생.   
fetch URL 오타 → 잘못된 요청 → Vite dev 서버 404 → index.html 리턴
```
...
const response = await fetch(`http://localhost:9000/api/events/${params.eventId}`)
```
쌍따옴표 제거하여 옳은 API 서버로 요청을 보내 문제 해결