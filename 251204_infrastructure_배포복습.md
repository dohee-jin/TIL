...
```
env.example 파일 작성   
.env 파일 말고 env.example 파일을 작성하면 된다.
...
api 개발 시 health 체크 추가하여 제대로 배포 되었는지 확인하는 과정이 추가되면 좋다.
...
도메인 연결되도 접속이 불가능 한데 9000번(설정한 값에 따라 다름)을 안열어둬서 접속할 수 없다.

ec2 > 보안그룹 > 인바운드규칙편집   
9000번 포트를 열어준다.   
anywhere로 하면 http로 접속이 가능하기 때문에 보안에 심각한 위험을 초래할 가능성이 높다.   
로드밸런서 설정할 때 만들어둔 대상타겟을 연결해줌   
![alt text](image-20.png)

api.~~~.~~ 도메인으로 접속해서 연결되는 지 확인한다.   
비정상일 경우 접속 여부를 다시 확인한다.
![alt text](image-21.png)
현재는 spring security에 의해 인증되지 않은 접근은 막고 401에러를 반환하고 있어 백엔드 배포가 완료되어도 비정상으로 출력된다.    

#### 이어서 할 작업
- OAUTH에 백엔드 API 도메인 등록
- OAUTH를 내 계정으로 생성하여 연결하기
- 프론트엔드 배포
