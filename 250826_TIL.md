...
#### 🛠️ 문제와 원인
- 문제: 배열에서 특정 요소를 삭제하려고 pop()을 사용했지만 원하던 요소가 아닌 배열 맨 끝 요소가 삭제됨
...
```   
좋은 해결방법은 아닌 것 같아 찾아보던 중 filter를 사용하여 제거하는 방법을 추천받았다. 
```
setGoals(prev => prev.filter(goal => goal.id !== id))
```
filter(), map() 등 배열고차함수를 활용하면 상태변경 시 원본을 변경하지 않고 새로운 배열, 객체를 생성하여 리액트의 불변성 원칙을 지키고 조건부 렌더링을 할 수 있다.
